...
sol, S, R = IP.solve(;   # solve -> arb. Mat. / solve1 -> high contrast formulation
    md = md, 
    material = IP.constantmaterial(0.2, 10000.0*IP.ε0), # IP.constant_xsplit(0.5, 3000.0*IP.ε0, 0.0008, 0.2, 10000.0*IP.ε0), # IP.constant_zsplit(0.5, 3000.0*IP.ε0, 0.0008, 0.2, 10000.0*IP.ε0), #  #, #IP.pwlinx([[1.0, 20.0],[40.0, 100.0],[200.0, 5.0]], nothing, [-md.body.L_x/2, -0.01/6, 0.01/6, md.body.L_x/2]), # , # ##IP.constant_zsplit(0.2, 0.0, 0.0, 0.0, 10000.0*IP.ε0), #     #IP.general_material(κ, nothing),  #  I  ,#, # #, #
